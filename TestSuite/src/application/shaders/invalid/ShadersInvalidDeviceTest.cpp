...
std::string ShadersInvalidDeviceTest::test()
{
	initialise();

	if (FAILED(result))
	{
		return "shaders invalid device test failed to initialise\n";
	}
	...
	return "shaders invalid device test failed\n";
}

void ShadersInvalidDeviceTest::cleanup(IUnknown* input)
{
	if (input)
	{
		input->Release();
...
void ShadersInvalidDeviceTest::initialise()
{
