...
PerSceneApplyWorldViewProjectionTest::PerSceneApplyWorldViewProjectionTest()
...
std::string PerSceneApplyWorldViewProjectionTest::test()
{
	initialiseD3D11();

	if (FAILED(result))
	{
		return "per scene apply world view projection test failed to initialise D3D11\n";
	}
	...
	if (!sceneBuffer)
	{
		return "per scene apply world view projection test failed to initialise scene buffer\n";
	}
	...
	if (FAILED(result))
	{
		return "per scene apply world view projection test failed to initialise read buffer\n";
	}
	...
	return "per scene apply world view projection test failed\n";
}

void PerSceneApplyWorldViewProjectionTest::initialiseCamera()
{
	for (int i{ 0 }; i < 8; i++)
	{
		camera[i] = (float)i;
...
void PerSceneApplyWorldViewProjectionTest::cleanup(IUnknown* input)
{
	if (input)
	{
		input->Release();
...
void PerSceneApplyWorldViewProjectionTest::initialiseD3D11()
{
...
void PerSceneApplyWorldViewProjectionTest::initialiseRead()
{
...
void PerSceneApplyWorldViewProjectionTest::initialiseOuput()
{
...
DirectX::XMMATRIX PerSceneApplyWorldViewProjectionTest::WVPCamera()
{
...
bool PerSceneApplyWorldViewProjectionTest::success()
{
